...
const makeWsRpcLayer = (currentSessionId: AuthSessionId) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const projectPackageJsonCatalog = yield* ProjectPackageJsonCatalog;
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.projectsListPackageJsonScripts]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsListPackageJsonScripts,
            projectPackageJsonCatalog.listPackageJsonScripts(input).pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectListPackageJsonScriptsError({
                    message:
                      cause instanceof Error
                        ? `Failed to list package.json scripts: ${cause.message}`
                        : "Failed to list package.json scripts",
                    cause,
                  }),
              ),
            ),
