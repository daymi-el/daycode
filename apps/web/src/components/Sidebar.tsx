...
import { getProviderDisplayName, ProviderIcon } from "./ProviderIcon";
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const providerLabel = getProviderDisplayName(thread.provider);
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5 text-left">
          ...
          {providerLabel && (
            <span
              ...
              title={providerLabel}
              data-testid={`thread-provider-icon-${thread.id}`}
              className="inline-flex shrink-0 items-center justify-center"
            >
              <ProviderIcon provider={thread.provider} aria-hidden="true" className="size-3.5" />
            </span>
          )}
  ...
  const handleProjectButtonContextMenu = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        const threadsToDelete = sidebarThreads;
        const threadCount = threadsToDelete.length;
        const confirmMessage =
          threadCount === 0
            ...
            : [
                `Remove project "${project.name}"?`,
                threadCount === 1
                  ? "This will also permanently delete 1 thread in this project."
                  : `This will also permanently delete all ${threadCount} threads in this project.`,
              ].join("\n");
...
function DaycodeWordmark() {
  return (
    <span
      aria-label="daycode"
      className="shrink-0 text-sm font-semibold lowercase leading-none tracking-tight text-foreground"
    >
      daycode
    </span>
  );
